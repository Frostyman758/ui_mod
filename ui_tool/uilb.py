...
def convert_layout_to_tpp(layout: UiLayoutBinary, path_map: Mapping[str, int]) -> UiLayoutBinary:
    """Promote a PC GZ layout to the TPP binary format using a path hash map."""
    if layout.version is not UiLayoutVersion.GZ:
        raise ValueError("Input layout is already in the TPP format")

    missing: List[str] = []
    hashed_paths: List[int] = []
    for entry in layout.path_entries:
        if not isinstance(entry, PathStringEntry):
            raise ValueError("GZ layouts must contain path strings, not numeric hashes")
        value = entry.value
        hashed = path_map.get(value)
        if hashed is None:
            missing.append(value)
        hashed_paths.append(hashed if hashed is not None else -1)

    if missing:
        formatted = "\n - ".join(missing)
        raise KeyError(
            "The bundled path hash map does not contain hashes for the following resources:"
            f"\n - {formatted}"
        )

    return UiLayoutBinary(
        version=UiLayoutVersion.TPP,
        models=layout.models,
        animations=layout.animations,
        cameras=layout.cameras,
        layouts=layout.layouts,
        str_codes=[entry.hash for entry in layout.str_codes],
        path_entries=hashed_paths,
    )


def convert_gz_to_tpp(input_path: Path, output_path: Path, path_map: Mapping[str, int]) -> None:
    """Convert a PC GZ ``.uilb`` file into a TPP ``.uilb`` using ``path_map``."""
    layout = UiLayoutBinary.from_bytes(input_path.read_bytes())
    converted = convert_layout_to_tpp(layout, path_map)
    output_path.write_bytes(converted.to_bytes())
