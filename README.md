...
python -m ui_tool convert 'uigb/PC GZ/SubtitlesBasic.uigb' converted.uigb
```
...
The `convert` command upgrades PC Ground Zeroes `.uilb` and `.uigb` archives to the TPP layout format so that you can reuse older resources in the newer engine.  The repository ships with a path hash map (for resolving layout references) and a strcode conversion table (for the `.uigb` graphs) that cover the resource paths contained in `ui format examples.zip`; if you want to convert files that reference new assets you can pass `--path-map` to point at a custom JSON file that maps path strings to their TPP hash values.
